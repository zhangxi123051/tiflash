...
FAIL_POINT_REGISTER(timeout_during_batch_cop)
...
#define FAIL_POINT_TRIGGER_TIMEOUT(fail_point, timeout) \
    fiu_do_on(fail_point, std::this_thread::sleep_for(timeout))
...
public:
    static void enableFailPoint(const String & fail_point_name)
    {
        ...
        FAIL_POINT_ENABLE(fail_point_name, timeout_during_batch_cop)
